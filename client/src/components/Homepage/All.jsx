...
import { useGetProductsQuery } from '../../api/loginApi';
import { useCart } from '../../CartContext';
// import { Helmet } from 'react-helmet';  // for whatsapp link share   [Future used]
...
const Cloth = (props) => {
  ...
  const handleShareViaWhatsapp = () => {
    const shareText = `${props.name} - ${props.price}$`;
    const encodedText = encodeURIComponent(shareText);
    const encodedUrl = encodeURIComponent(props.url);
    const whatsappMessage = `${encodedText}%0A${encodedUrl}`; // %0A represents a line break
...
function All() {
  ...
  if (isSuccess && data) {
    return (
      <>
        {/* <Helmet>
        <title>ShopNex [Online Market of India]................</title>
          <meta property="og:title" content="Your Website Title" />
          <meta property="og:description" content="Description of your website" />
          {data.map((product, index) => (
            <meta key={index} property="og:image" content={`http://localhost:5173/All/${product.image}`} />
          ))}
