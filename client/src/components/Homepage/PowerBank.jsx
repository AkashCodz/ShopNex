...
import { useGetPowerBankQuery } from '../../api/loginApi';
...
const Cloth = (props) => {
  ...
  return (
    <>
      <div className="bg-orange-500 shadow-lg rounded-lg overflow-hidden">
        <img src={props.url} alt="Product 1" className="w-44 h-44 mx-auto rounded-xl py-2" />
        <div className="text-center">
          <h3 className="text-base sm:text-lg font-bold">{props.name}</h3>
          <p className="text-gray-700">{props.price}k</p>
...
function All() {
  const { data, error, isLoading, isSuccess } = useGetPowerBankQuery();
  console.log(data);
