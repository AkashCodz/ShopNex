...
function SignIn() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await postLoginMutation(detail);
      console.log("Login successful", data);

      const username = data.body[1];
      console.log('username',username)
      dispatch(setUser(username));

      if(data.data === "Success"){
        navigate("/all"); // Redirect on success
      }
    } catch (error) {
      console.error("Login error", error);
      if (error.response && error.response.status === 401) {
        // Handle unauthorized error (e.g., display specific message)
      } else {
        // Handle other errors (e.g., generic error message)
      }
    }
  };
    // if(responseInfo.isSuccess){
    //   const username = responseInfo.data.body[1];  
    //   console.log('Username',username)
    // }
      ...
      <div className="font-[sans-serif] text-[#333]">
        <div className="min-h-screen flex fle-col items-center justify-center py-6 px-4">
          <div className="grid md:grid-cols-2 items-center gap-10 max-w-6xl w-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 max-w-md md:ml-auto max-md:mx-auto w-full">
